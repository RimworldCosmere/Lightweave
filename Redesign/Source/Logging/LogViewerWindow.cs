...
internal sealed class LogViewerWindow : LightweaveWindow {
            ...
            children: outer => outer.Add(HStack.Create(
                ...
                children: row => {
                    row.AddHug(IconButton.Create(
                        icon: Glyph.Create(
                            state.ChannelsOpen ? Icons.Phosphor.SidebarSimple : Icons.Phosphor.Sidebar,
                ...
                children: row => {
                    ...
                    if (hasChildren) {
                        row.AddHug(Glyph.Create(
                            expanded ? Icons.Phosphor.CaretDown : Icons.Phosphor.CaretRight,
                            style: new Style {
    ...
    private LightweaveNode BuildLogRow(LogEntry entry, Action invalidate) {
        ...
        string mod = string.IsNullOrEmpty(entry.Mod) ? "" : "[" + entry.Mod + "]";
        ...
        LightweaveNode body = HStack.Create(
            ...
            children: row => {
                ...
                if (hasMod) {
                    row.AddHug(Text.Create(
                        mod,
                        style: new Style { FontFamily = RenderContext.Current.Theme.GetFont(FontRole.Mono), FontSize = new Rem(0.65625f), TextColor = ThemeSlot.TextSecondary, LetterSpacing = Tracking.Of(0.1f) }
                    ));
                }
                if (hasSource) {
                    row.AddHug(Text.Create(
    ...
    private static LightweaveNode DetailRow(string keyLabel, string value, ThemeSlot valueSlot, bool wrap = false) {
        return KeyValue.Create(
            keyLabel.ToUpperInvariant(),
            Text.Create(
                value,
                wrap: wrap,
                style: new Style { FontFamily = RenderContext.Current.Theme.GetFont(FontRole.Mono), FontSize = new Rem(0.71875f), TextColor = valueSlot }
            ),
