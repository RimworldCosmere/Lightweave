...
public abstract class LightweaveWindow : Verse.Window {
    ...
    private void HandleWindowDrag(Rect inRect) {
        ...
        if (isWindowDragging) {
            ...
            Rect next = windowRect;
            next.x = dragStartRect.x + delta.x;
            next.y = dragStartRect.y + delta.y;
            next.x = Mathf.Clamp(next.x, 0f, Verse.UI.screenWidth - dragStartRect.width);
            next.y = Mathf.Clamp(next.y, 0f, Verse.UI.screenHeight - dragStartRect.height);
            windowRect = next;
        ...
        if (raw != EventType.MouseDown
            ...
            || LightweaveHitTracker.IsOver(screenTL)) {
            return;
        }
    ...
    private void UpdateCursor(Rect inRect) {
        ...
        if (dragRect.HasValue && dragRect.Value.Contains(mouse) && !LightweaveHitTracker.IsOver(screenPoint)) {
